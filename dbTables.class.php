...
class dbTables {
   ...
   * Make result rows
   *
   * This method's $rowdesc has a wild card, like this: `'<tr><td>*</td></tr>'`.
   * It can also have a header like `'<table><thead>%<th>*</th>%</thead>'`.
   * The header delimiter is always `%`.
   * In both cases the fields from the query will replace the '*'.
   * Make the `$query` fields what you want to appear in the table header using the 'as' keywork.
   *
   * @param string $query
   * @param string $rowdesc
   * @param array $extra
   *   $extra[delim] is an array|string with the delimiter,
   ...
   *     then returned is an array({the row string}, {result}, {num}, {header},
  ...
  public function makeresultrows(string $query, string $rowdesc=null, array $extra=null):mixed {
    ...
    // If no array element 'delim' use a default.
    
    $delim = $extra['delim'] ?? ['<td>', '</td>'];

    // If no $rowdesc use a default.
    
    $rowdesc = $rowdesc ?? "<tr><td>*</td></tr>"; 
    ...
    $realToAlias = [];
    $columnClasses = [];
    $firstRow = true; // this is the fisrt row.
    $mkrow = true;
    ...
    while($row = $this->db->fetchrow($result, 'assoc')) {
      if($firstRow) {
        ...
        for($i = 0; $i < $count; $i++) {
          ...
          $real = $meta['name'];
          $alias = $keys[$i]; // The key is the alias, i.e. 'the ip'=>'1.1.1.4'
          $realToAlias[$real] = $alias;
          $columnClasses[$alias] = $prefix . $real;
        }
